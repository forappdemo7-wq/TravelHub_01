      ...
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-2">
                <span className="flex items-center gap-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 px-3 py-1 rounded-full text-sm">
                  <Star className="w-4 h-4 fill-yellow-500 text-yellow-500" />
                  {restaurant.rating} ({restaurant.reviewsCount} reviews)
                </span>
                <span className="text-sm text-zinc-500">{restaurant.cuisine}</span>
                <span className="text-sm text-zinc-500">•</span>
                <span className="text-sm text-zinc-500">{restaurant.location}</span>
              </div>
            </div>
            ...
            {restaurant.menuHighlights && restaurant.menuHighlights.length > 0 && (
              <div>
                <h3 className="font-semibold text-lg">Menu Highlights</h3>
                <ul className="list-disc list-inside text-zinc-600 dark:text-zinc-400 space-y-1 mt-2">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {restaurant.dietaryOptions.map((d) => (
                    <span
                      key={d}
                      className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-3 py-1 rounded-full text-sm"
                    >
                      {d}
                    </span>
                  ))}
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-lg border border-zinc-200 dark:border-zinc-800 space-y-4">
              <h3 className="font-bold text-xl">Information</h3>
              <div className="space-y-3 text-sm">
                <div className="flex items-start gap-3">
                  <DollarSign className="w-5 h-5 text-zinc-400 shrink-0 mt-0.5" />
                  <div>
            ...
            {restaurant.bookingUrl && (
              <a
                ...
                className="block w-full text-center border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-sm font-medium py-2 px-4 rounded-xl transition flex items-center justify-center gap-2"
              >
                Book via Partner <ExternalLink className="w-4 h-4" />
              </a>
            )}
            ...
            {restaurant.destinationId && (
              <Link
                ...
                className="block w-full text-center border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 font-medium py-3 px-6 rounded-2xl transition"
              >
                View Destination
              </Link>
            )}
